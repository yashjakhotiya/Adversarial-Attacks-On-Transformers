...
from tensorflow.keras.layers import Add, Dense, Dropout, Embedding, Layer, LayerNormalization, Multiply, Permute, Reshape, Concatenate, Lambda, concatenate
...
class Transformer:
    def __init__(self,
        ...
        self.decoder = Decoder(num_layers=num_layers,
                               ...
                               dropout_rate=dropout_rate,
                               ffn_activation=ffn_activation,
                               scope="%s_decoder" % scope)

        self.final_layer = Dense(target_vocab_size,
                                 activation=None,
                                 name="%s_dense" % scope)

        self.padding_mask = PaddingMask(name="%s_padding_mask" % scope)
        self.lookahead_mask = PaddingAndLookaheadMask(
            name="%s_lookahead_mask" % scope)
...
class Decoder:
    def __init__(self,
        ...
        self.embedding = Embedding(input_dim=vocab_size,
                                   output_dim=d_model//3,
                                   name="%s_embedding" % scope)
        self.pos_encoding = PositionalEncoding(d_model,
                                               name="%s_positional_encoding" %
                                               scope)
    ...
    def __call__(self, x, enc_output, lookahead_mask, padding_mask):
        first = Lambda(lambda x: x[:, :, 0])(x)
        second = Lambda(lambda x: x[:, :, 1])(x)
        third = Lambda(lambda x: x[:, 2])(x)
        first_embed = self.embedding(first)
        second_embed = self.embedding(second)
        third_embed = self.embedding(third)
        x = concatenate([first_embed, second_embed, third_embed])
        x = MultiplyConstant(self.d_model, name="%s_multiply" % self.scope)(x)
...
class Encoder:
    def __init__(self,
        ...
        self.dropout = Dropout(dropout_rate, name="%s_dropout" % self.scope)
        self.multiply = MultiplyConstant(self.d_model, name="%s_multiply" % self.scope)
        self.add = Add(name="%s_add" % self.scope)
    ...
    def embed_only(self, x):
        ...
        second_embed = self.embedding(second)
        third_embed = self.embedding(third)
        return concatenate([first_embed, second_embed, third_embed])

    def encode_from_embedded(self, embedded, padding_mask):
        x = self.multiply(embedded)
        x = self.add([x, self.pos_encoding(x)])
    ...
    def __call__(self, x, padding_mask):
        embedded = self.embed_only(x)
        return self.encode_from_embedded(embedded, padding_mask)
...
class DecoderLayer:
    def __init__(self,
        ...
        self.mha1 = MultiHeadAttention(d_model,
                                       num_heads,
                                       scope="%s_multi_head_attention_1" %
                                       scope)
        self.mha2 = MultiHeadAttention(d_model,
                                       num_heads,
                                       scope="%s_multi_head_attention_2" %
                                       scope)
        self.ffn = PointwiseFeedForwardNetwork(
            d_model,
            d_ff,
            activation=ffn_activation,
            scope="%s_pointwise_feed_forward_network" % scope)

        self.layernorm1 = LayerNormalization(epsilon=1e-6,
                                             name="%s_layer_norm_1" % scope)
        self.layernorm2 = LayerNormalization(epsilon=1e-6,
                                             name="%s_layer_norm_2" % scope)
        self.layernorm3 = LayerNormalization(epsilon=1e-6,
                                             name="%s_layer_norm_3" % scope)
...
class PointwiseFeedForwardNetwork:
    def __init__(self,
        ...
        self.dense_1 = Dense(d_ff,
                             ...
                             name="%s_dense_1" % scope)
        self.dense_2 = Dense(d_model,
                             activation=None,
                             name="%s_dense_2" % scope)
...
class MultiHeadAttention:
    def __init__(self, d_model, num_heads, scope="multi_head_attention"):
        ...
        self.reshape_output = Reshape((-1, d_model),
                                      name="%s_reshape_output" % scope)

        self.transpose_output = Permute((2, 1, 3),
                                        name="%s_transpose_output" % scope)
