...
for file in os.listdir(P.benign_exe_tokenized):
    tok_path = os.path.join(P.benign_exe_tokenized, file)
    exe_path = os.path.join(P.benign_exe, file)
    if os.path.getsize(tok_path) == 0:
        continue
    benign_files.append((tok_path, exe_path))
print("Length of benign files = {}".format(len(benign_files)))

malicious_files = []
for file in os.listdir(P.malicious_exe_tokenized):
    tok_path = os.path.join(P.malicious_exe_tokenized, file)
    exe_path = os.path.join(P.malicious_exe, file)
    if os.path.getsize(tok_path) == 0:
        continue
    malicious_files.append((tok_path, exe_path))
print("Length of malicious files = {}".format(len(malicious_files)))

data_files = []
for tok_path, exe_path in benign_files:
    data_files.append((tok_path, exe_path, 0))

for tok_path, exe_path in malicious_files:
    data_files.append((tok_path, exe_path, 1))
...
class TrainDataLoader(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(idx*self.batch_size, (idx+1)*self.batch_size):
            ...
            single_static_feature = get_string_feature_vector_for_file(train_files[i][1]) + get_dll_feature_vector_for_file(train_files[i][1])
            single_static_feature = np.array(single_static_feature)
            static_feature_batch.append(single_static_feature)

            labels_batch.append(train_files[i][2])
...
class ValDataLoader(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(idx*self.batch_size, (idx+1)*self.batch_size):
            ...
            single_static_feature = get_string_feature_vector_for_file(val_files[i][1]) + get_dll_feature_vector_for_file(val_files[i][1])
            single_static_feature = np.array(single_static_feature)
            static_feature_batch.append(single_static_feature)

            labels_batch.append(val_files[i][2])
...
class TestDataLoader(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(idx*self.batch_size, (idx+1)*self.batch_size):
            ...
            single_static_feature = get_string_feature_vector_for_file(test_files[i][1]) + get_dll_feature_vector_for_file(test_files[i][1])
            single_static_feature = np.array(single_static_feature)
            static_feature_batch.append(single_static_feature)

            labels_batch.append(test_files[i][2])
